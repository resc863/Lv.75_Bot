...
async def on_message(message):
    if message.content.startswith("!역할"):
        ...
        embed.add_field(name="멘션 거부", value="레인보우 식스 관련 멘션을 받지 않으려면 첫번째 버튼을 누르세요", inline=False)
        message = embed
        ...
        embed.add_field(name="멘션 거부", value="데스티니 2 관련 멘션을 받지 않으려면 첫번째 버튼을 누르세요", inline=False)
        message = embed
        await client.send_message(channel, embed=embed)
        ...
        embed.add_field(name="멘션 거부", value="GTA5 관련 멘션을 받지 않으려면 첫번째 버튼을 누르세요", inline=False)
        message = embed
        await client.send_message(channel, embed=embed)
        ...
        embed.add_field(name="멘션 거부", value="마인크래프트 관련 멘션을 받지 않으려면 첫번째 버튼을 누르세요", inline=False)
        message = embed
        await client.send_message(channel, embed=embed)
        await client.add_reaction(message, emoji="\U0001F590")
        await client.add_reaction(message, emoji="\U00002706")
        ...
        embed.add_field(name="멘션 거부", value="콜오브듀티 관련 멘션을 받지 않으려면 첫번째 버튼을 누르세요", inline=False)
        message = embed
        await client.send_message(channel, embed=embed)
        await client.add_reaction(message, emoji="\U0001F440")
        await client.add_reaction(message, emoji="\U0001F90F")

    if message.content.startswith("!가이드"):  
        channel = client.get_channel('638039195234992155')

        embed = discord.Embed(title="안내", description="다음을 읽고 버튼을 눌러 주십시오.", color=0x00ff00)
        message = embed
        await client.send_message(channel, embed=embed)
